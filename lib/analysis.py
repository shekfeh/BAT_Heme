...
def fe_openmm(
    ...
    fe_a = fe_bd = fe_t = fe_m = fe_n = fe_v = fe_e = fe_c = fe_r = fe_l = fe_f = (
        fe_w
    ) = fe_vs = fe_es = fe_x = 0
    fb_a = fb_bd = fb_t = fb_m = fb_n = fb_v = fb_e = fb_c = fb_r = fb_l = fb_f = (
        fb_w
    ) = fb_es = fb_vs = fb_x = 0
    sd_a = sd_bd = sd_t = sd_m = sd_n = sd_v = sd_e = sd_c = sd_r = sd_l = sd_f = (
        sd_w
    ) = sd_vs = sd_es = sd_x = 0
        ...
        if (
            ...
            if comp == "t" or comp == "m":
                ...
                with open("disang.rest", "r") as f_in:
                    ...
                    k_a = rest[3]
                    fe_bd = fe_int_op(
                        r0, a1_0, t1_0, a2_0, t2_0, t3_0, k_r, k_a, temperature
                    )
            ...
            if out_file.exists():
                with open(out_file, "r") as f_in:
                    ...
                    for k in range(0, len(lines)):
                        ...
                        if (
                            splitdata[0].strip() == "Relative"
                            and splitdata[6].strip() == "whole"
                        ):
                            if comp == "c":
            ...
            if dec_int == "mbar":
                ...
                if K != ti_points:
                    print(
                        "Error: Missing simulation data for TI-GQ for the "
                        + comp
                        + " component of the "
                        + pose
                        + " calculation"
    ...
    for k in range(0, blocks):
        ...
        for i in range(0, len(components)):
            ...
            if (
                ...
                with open("./" + comp + "-comp/output.dat", "r") as f_in:
                    lines = (line.rstrip() for line in f_in)
                    lines = list(
                        line for line in lines if "Relative" in line and "block" in line
                    )
                    splitdata = lines[k].split()
                    if comp == "c":
                ...
                comp == "v" or comp == "e" or comp == "f" or comp == "w" or comp == "x"
            ):
                if dec_method == "dd":
                    os.chdir(dec_method)
                if dec_method == "sdr" or dec_method == "exchange":
                ...
                if dec_int == "mbar":
                    with open("./" + comp + "-comp/output.dat", "r") as f_in:
                        ...
                        lines = list(
                            line
                            for line in lines
                            if "Relative" in line and "block" in line
                        )
                        splitdata = lines[k].split()
                        if comp == "e" and dec_method == "dd":
                            ...
                            dec_method == "sdr" or dec_method == "exchange"
                        ):
                    ...
                    while os.path.isfile(filename):
                        K = K + 1
                        filename = (
                            "./" + comp + "-comp/" + comp + "%02.0f/output.dat" % K
                        )
                    deltagop = 0
                    for j in range(K):
                        filename = (
                            "./" + comp + "-comp/" + comp + "%02.0f/output.dat" % j
                        )
                        wind_d = 0
                        if os.path.exists(filename):
                            with open(filename, "r") as f_in:
        ...
        blck_dd = -1 * (
            fb_a + fb_l + fb_t + fb_e + fb_v + fb_w + fb_f + fb_bd + fb_c + fb_r
        )
    ...
    # Write final results
    total_dd = -1 * (
        fe_a + fe_l + fe_t + fe_e + fe_v + fe_w + fe_f + fe_bd + fe_c + fe_r
    )
    ...
    sd_merg_dd = math.sqrt(
        sd_m**2 + sd_e**2 + sd_v**2 + sd_w**2 + sd_f**2 + sd_bd**2 + sd_n**2
    )
    sd_sdr = math.sqrt(
        sd_a**2 + sd_l**2 + sd_t**2 + sd_es**2 + sd_vs**2 + sd_bd**2 + sd_c**2 + sd_r**2
    )
    sd_exc = math.sqrt(
        sd_a**2 + sd_l**2 + sd_t**2 + sd_es**2 + sd_x**2 + sd_bd**2 + sd_c**2 + sd_r**2
    )
    ...
    cit = "on"
    resfile.write(
        "Total simulation time (based on input file): %6.1f nanoseconds\n\n"
        % total_time
    )
    if cit == "on":
        resfile.write("Please cite:\n\n")
        resfile.write(
            "G. Heinzelmann, D. J. Huggins and M. K. Gilson (2024). “BAT2: an Open-Source Tool for Flexible, Automated, and Low Cost Absolute Binding Free Energy Calculations”. Journal of Chemical Theory and Computation, 20, 6518.\n\n"
        )
        resfile.write(
        ...
        resfile.write(
            'D. J. Huggins (2022) "Comparing the Performance of Different AMBER Protein Forcefields, Partial Charge Assignments, and Water Models for Absolute Binding Free Energy Calculations." Journal of Chemical Theory and Computation, 18, 2616.\n\n'
        )
    resfile.close()
...
def fe_values(
    ...
    for i in components:
        if (
            ...
            total_time = (
                total_time
                + (dic_steps1[i] + dic_steps2[i]) * len(attach_rest) * float(dt) / 1000
            )
        else:
            total_time = (
                total_time
                + (dic_steps1[i] + dic_steps2[i]) * len(lambdas) * float(dt) / 1000
            )
    #    print(total_time)
        ...
        if (
            ...
            for j in range(0, len(attach_rest)):
                ...
                for k in range(0, blocks):
                    ...
                    for t in range(
                        k * int(round(len(data) // blocks)),
                        (k + 1) * int(round(len(data) // blocks)),
                    ):
                        fout.write(data[t])
                    fout.close()
                os.chdir("../")
        ...
        if (
            ...
            if dec_int == "mbar":
                if comp == "e" and dec_method == "dd":
                    ...
                    for k in range(0, blocks):
                        with open(
                            "./data/mbar-" + comp + "-b%02d.dat" % (k + 1), "r"
                        ) as f_in:
                            lines = (line.rstrip() for line in f_in)
                            lines = list(line for line in lines if line)
                            data = lines[-1]
        ...
        for i in range(0, len(components)):
            ...
            if (
                ...
                if dec_method == "sdr" or dec_method == "exchange":
                    os.chdir("sdr")
                with open(
                    "./data/" + dec_int + "-" + comp + "-b%02d.dat" % (k + 1), "r"
                ) as f_in:
                    lines = (line.rstrip() for line in f_in)
                    lines = list(line for line in lines if line)
                    data = lines[-1]
...
def fe_mbar(comp, pose, mode, rest_file, temperature):
    ...
    def _normalize_mbar_f(ret):
        """
        Make PyMBAR getFreeEnergyDifferences() output robust:
        - If returns array → (Deltaf, nan-array)
        - If returns (Deltaf, dDeltaf) → cast to arrays
        - If returns (Deltaf,) or weird tuple → coerce sensibly
        """
        if isinstance(ret, tuple):
            if len(ret) == 2:
                Deltaf, dDeltaf = ret
                Deltaf = _np.asarray(Deltaf)
                dDeltaf = _np.asarray(dDeltaf)
                return Deltaf, dDeltaf
            elif len(ret) == 1:
                Deltaf = _np.asarray(ret[0])
                dDeltaf = _np.full_like(Deltaf, _np.nan)
                return Deltaf, dDeltaf
            else:
                # unexpected tuple shape; treat as array and fill NaNs for sigma
                Deltaf = _np.asarray(ret)
                dDeltaf = _np.full_like(Deltaf, _np.nan)
                return Deltaf, dDeltaf
        else:
            Deltaf = _np.asarray(ret)
    ...
    val = np.zeros(
        [N_max, K, R], np.float64
    )  # value of the restrained variable at each frame n
    g = np.zeros([K], np.float64)
    u = np.zeros([N_max], np.float64)

    # === DEBUG: window directory sanity, signature, and column checks ===
    import json
    import glob

    # Which window dirs actually exist (contiguous or not)
    win_dirs = sorted(
        [
            os.path.basename(p)
            for p in glob.glob(f"./{comp}[0-9][0-9]")
            if os.path.isdir(p)
        ]
    )
    K_dirs = len(win_dirs)
    print(
        f"[DEBUG {comp}-{mode}] Window dirs discovered: {win_dirs}  (K_dirs={K_dirs}, K_from_files={K})"
    )
    if K != K_dirs:
        print(
            f"[DEBUG {comp}-{mode}] WARNING: K detected from '{rest_file}' ({K}) != number of directories ({K_dirs}). "
            f"Stray/missing window suspected."
        )

    # Build a reference "signature" (order + labels) from comp00/disang.rest
    def restraint_signature(disang_path):
        sig = []
        with open(disang_path, "r") as fin:
            for line in fin:
                cols = line.split()
                if cols and cols[-1] in (
                    "#Lig_TR",
                    "#Lig_C",
                    "#Lig_D",
                    "#Rec_C",
                    ...
                    # include the atom list token to catch order/type changes
                    sig.append(f"{cols[-1]}|{cols[2]}")
        return sig

    ref_disang = f"./{comp}00/disang.rest"
    ref_sig = restraint_signature(ref_disang)
    print(f"[DEBUG {comp}-{mode}] R_from_signature={len(ref_sig)} (should equal R={R})")
    if len(ref_sig) != R:
        print(
            f"[DEBUG {comp}-{mode}] WARNING: R from signature ({len(ref_sig)}) != computed R ({R})."
        )

    # Compare every window's signature & data-file column count (NF) vs expected R+1
    debug_per_window = []
    for ktest in range(K):
        ddir = f"./{comp}{ktest:02d}"
        dfile = f"{ddir}/{rest_file}"
        sfile = f"{ddir}/disang.rest"
        entry = {"window": f"{comp}{ktest:02d}", "exists": os.path.exists(dfile)}
        # signature compare
        if os.path.exists(sfile):
            sig = restraint_signature(sfile)
            entry["sig_len"] = len(sig)
            entry["sig_match"] = sig == ref_sig
        else:
            entry["sig_len"] = None
            entry["sig_match"] = False
        ...
        entry["rows"] = n_rows
        entry["nf"] = nf
        entry["nf_expected"] = R + 1 if R is not None else None
        entry["nf_ok"] = (nf == (R + 1)) if nf is not None and R is not None else False
        debug_per_window.append(entry)

    print(
        "[DEBUG {0}-{1}] window_summary:\n{2}".format(
            comp, mode, json.dumps(debug_per_window, indent=2)
        )
    )

    # Hard preflight guards before MBAR work starts
    bad = [e["window"] for e in debug_per_window if e["rows"] == 0]
    if bad:
        raise RuntimeError(
            f"[{comp}-{mode}] Empty '{rest_file}' (0 data rows) in windows: {bad}"
        )

    bad_nf = [
        e
        for e in debug_per_window
        if e["nf"] is not None
        and e["nf_expected"] is not None
        and e["nf"] != e["nf_expected"]
    ]
    if bad_nf:
        raise RuntimeError(
            f"[{comp}-{mode}] Column mismatch in '{rest_file}': "
            + ", ".join(
                [
                    f"{e['window']} (NF={e['nf']}, expected {e['nf_expected']})"
                    for e in bad_nf
                ]
            )
        )

    bad_sig = [e["window"] for e in debug_per_window if e["sig_match"] is False]
    if bad_sig:
        print(
            f"[DEBUG {comp}-{mode}] WARNING: Restraint signature differs from {comp}00 in: {bad_sig}"
        )
    ...
    for k in range(K):
        ...
        if comp != "u":  # Attach/Release Restraints
            if rfc[k, 0] == 0:
                tmp = (
                    np.ones([R], np.float64) * 0.001
                )  # CHECK THIS!! might interfere on protein attach
                u[0 : N[k]] = np.sum(
                    beta * tmp[0:R] * ((val[0 : N[k], k, 0:R] - req[k, 0:R]) ** 2),
                    axis=1,
                )
            else:
                u[0 : N[k]] = np.sum(
                    beta * rfc[k, 0:R] * ((val[0 : N[k], k, 0:R] - req[k, 0:R]) ** 2),
                    axis=1,
                )
        ...
        print(
            "Processed Window %5.0f.  N= %12.0f.  g= %10.3f   Neff= %12.0f"
            % (k, N[k], g[k], Neff[k])
        )

    #### Debug###
    print(
        f"[DEBUG {comp}-{mode}] {comp}{k:02d}: N={int(N[k])}, g={g[k]:.3f}, Neff={int(Neff[k])}"
    )
    if N[k] == 0:
        raise RuntimeError(
            f"[{comp}-{mode}] {comp}{k:02d}: N==0 (no usable restraint data)."
        )
    if int(np.sum(Neff)) == 0:
        raise RuntimeError(
            f"[{comp}-{mode}] All Neff are zero; nothing to pass to MBAR."
        )
    ### Debug End###
        ...
        if mode == "sub":  # subsampling
            ...
            for l in range(K):  # all samples
                if comp != "u":  # Attach Restraints
                    ...
                    Upot[k, l, 0 : Neff[k]] = (
                        beta * rfc[l, 0] * ((val[0 : Neff[k], k, 0] - req[l, 0]) ** 2)
                    )

    val = []

    # --- EXTRA DEBUG: print inputs MBAR will see ---
    print(f"[DEBUG {comp}-{mode}] K={K}, R={R}")
    print(f"[DEBUG {comp}-{mode}] N_k (raw): {N.tolist()}")
    print(f"[DEBUG {comp}-{mode}] Neff (used): {Neff.tolist()}")
    # Basic stats of self-energies per state (u_kk) to catch crazy scales
    for k in range(K):
        if Neff[k] > 0:
            u_self = Upot[k, k, 0 : Neff[k]]
            print(
                f"[DEBUG {comp}-{mode}] u_self[{k}] min/med/max = "
                f"{float(np.min(u_self)):.4f} / {float(np.median(u_self)):.4f} / {float(np.max(u_self)):.4f}"
            )
        else:
            print(f"[DEBUG {comp}-{mode}] u_self[{k}] has Neff=0")

    ####### Running MBAR###############

    #    print("Running MBAR... ")
    #    mbar = MBAR(Upot, Neff, verbose=True)
    #
    #    # Optional: skip uncertainty to avoid covariance/normalization check during debugging.
    #    compute_unc = True
    #    if os.environ.get("BAT_DEBUG_SKIP_UNCERT", "0") == "1":
    #        compute_unc = False
    #        print(
    #            "[DEBUG] BAT_DEBUG_SKIP_UNCERT=1 → compute_uncertainty=False for getFreeEnergyDifferences"
    #        )
    #
    #    print("Calculate Free Energy Differences Between States")
    #    try:
    #        Deltaf, dDeltaf = mbar.getFreeEnergyDifferences(compute_uncertainty=compute_unc)
    #    except Exception as e:
    #        print("[DEBUG] MBAR getFreeEnergyDifferences failed:", repr(e))
    #        try:
    #            ov = mbar.computeOverlap()
    #            print("[DEBUG] Overlap matrix:\n", ov.get("matrix", ov))
    #        except Exception as e2:
    #            print("[DEBUG] Overlap computation also failed:", repr(e2))
    #        # Re-raise so your current control flow stays the same.
    #        raise

    print("Running MBAR... ")
    mbar = MBAR(Upot, Neff, verbose=True)

    # Try with uncertainties first; if it fails, fall back to f-only.
    try:
        ret = mbar.getFreeEnergyDifferences(compute_uncertainty=True)
    except Exception as e_unc:
        print("[DEBUG] Uncertainty step failed:", repr(e_unc))
        ret = mbar.getFreeEnergyDifferences(compute_uncertainty=False)
        print("[DEBUG] Proceeding with Δf only; dΔf will be NaN.")

    Deltaf, dDeltaf = _normalize_mbar_f(ret)

    print(
        f"[DEBUG] Deltaf type={type(Deltaf)}, shape={getattr(Deltaf,'shape',None)}; "
        f"dDeltaf type={type(dDeltaf)}, shape={getattr(dDeltaf,'shape',None)}"
    )

    # (optional) overlap dump to help diagnose overlap vs. input mismatch
    try:
        ov = mbar.computeOverlap()
        ovmat = ov.get("matrix", ov)
        _np.savetxt(f"./data/mbar-{comp}-{mode}-overlap.txt", ovmat, fmt="%.6f")
        print("[DEBUG] Overlap written to", f"./data/mbar-{comp}-{mode}-overlap.txt")
    except Exception as e2:
        print("[DEBUG] Overlap computation failed:", repr(e2))
...
def fe_dd(
    comp, pose, mode, lambdas, weights, dec_int, dec_method, rest_file, temperature
):
    ...
    if dec_int == "ti":
        ...
        for k in range(K):
            print(
                "%10.5f %10.5f %10.5f"
                % (lambdas[k], Deltaf[0, k] / beta, dDeltaf[0, k] / beta)
            )
        print("\n\n")
