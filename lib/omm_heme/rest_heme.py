...
kB = (
    0.008314472471220214
    * unit_definitions.kilojoules_per_mole
    / unit_definitions.kelvin
)
...
# === Fe-related (heme, Cys, Glu, H-bond) distance restraints ===
atoms_fe = []
eq_fe = []
fcn_fe = []

print("Fe-related restraints:")
print("")

with open("disang.rest", "r") as f_in:
    lines = (line.rstrip() for line in f_in)
    lines = list(line for line in lines if "#Fe_rest" in line)
    ...
    for line in lines:
        ...
        atom_str = [kv for kv in splitdata if kv.startswith("iat=")][0]
        atoms = atom_str.split("=")[1].split(",")

        # Only take r2 and rk2 for midpoint and force constant
        eq_val = 0.5 * (float(params["r2"]) + float(params["r3"]))
        k_val = 0.5 * (float(params["rk2"]) + float(params["rk3"]))

        atoms = [int(a.strip()) - 1 for a in atoms if a.strip()]
        if len(atoms) == 2:
            atoms_fe.append(atoms)
            eq_fe.append(eq_val)
            fcn_fe.append(k_val)

print("Fe restraints parsed:", len(atoms_fe))
print("")
...
system.addForce(
    MonteCarloBarostat(
        1 * unit_definitions.atmospheres, TMPRT * unit_definitions.kelvin, 25
    )
)
...
# --- Fe-related harmonic bond restraints (permanent, not scaled by lambda) ---
fe_force = CustomBondForce("(K/2)*(r - r0)^2")
fe_force.addPerBondParameter("r0")
fe_force.addPerBondParameter("K")
fe_force.setForceGroup(system.getNumForces())  # optional

print("Fixed Fe-related restraints:")
print("")
for i in range(len(atoms_fe)):
    bondlength = eq_fe[i] * unit_definitions.angstroms
    bondforce = (
        fcn_fe[i]
        * unit_definitions.kilocalorie_per_mole
        / unit_definitions.angstroms**2
    )
    fe_force.addBond(atoms_fe[i][0], atoms_fe[i][1], [bondlength, bondforce])
    print(
        f"fe_force.addBond({atoms_fe[i][0]}, {atoms_fe[i][1]}, "
        f"[{bondlength.value_in_unit(unit_definitions.angstroms):.3f}, "
        f"{bondforce.value_in_unit(unit_definitions.kilocalorie_per_mole / unit_definitions.angstroms**2):.3f}])"
    )
print("")

system.addForce(fe_force)

#### The decoupling restraints ###
...
if comp == "l" or comp == "t":
    ...
    for i in range(0, len(atoms_pc)):
        if len(atoms_pc[i]) == 2:
            ...
            harmonicforce.addBond(
                atoms_pc[i][0], atoms_pc[i][1], [bondlength, bondforce]
            )
            print(
                "harmonicforce.addBond("
                + str(atoms_pc[i][0])
            ...
            torsionforce.addTorsion(
                atoms_pc[i][0],
                atoms_pc[i][1],
                atoms_pc[i][2],
                atoms_pc[i][3],
                [1, eq_pc[i] * unit_definitions.radian, torsionconst],
            )
            print(
                "torsionforce.addTorsion("
                + str(atoms_pc[i][0])
        ...
        for i in range(0, len(atoms_lc)):
            if len(atoms_lc[i]) == 4:
                ...
                torsionforce.addTorsion(
                    ...
                    atoms_lc[i][2],
                    atoms_lc[i][3],
                    [1, eq_lc[i] * unit_definitions.radian, torsionconst],
                )
                print(
                    "torsionforce.addTorsion("
                    + str(atoms_lc[i][0])
...
if comp == "t" or comp == "m":
    ...
    for i in range(0, len(atoms_tr)):
        if len(atoms_tr[i]) == 2:
            ...
            harmonicforce.addBond(
                atoms_tr[i][0], atoms_tr[i][1], [bondlength, bondforce]
            )
            print(
                "harmonicforce.addBond("
                + str(atoms_tr[i][0])
            ...
            angleconst = (
                fcn_tr[i]
                * unit_definitions.kilocalorie_per_mole
                / unit_definitions.radian**2
            )
            angleforce.addAngle(
                atoms_tr[i][0],
                atoms_tr[i][1],
                atoms_tr[i][2],
                [restrainedangle, angleconst],
            )
            print(
                "angleforce.addAngle("
                + str(atoms_tr[i][0])
            ...
            torsionforce.addTorsion(
                ...
                atoms_tr[i][2],
                atoms_tr[i][3],
                [1, eq_tr[i] * unit_definitions.radian, torsionconst],
            )
            print(
                "torsionforce.addTorsion("
                + str(atoms_tr[i][0])
...
if dum_atom >= 1:
    ...
    print(
        ...
        + str(com_atoms[0])
        + ", "
        + str(bondParameters[0])
        + ", "
        + str(bondParameters[1])
        + ", "
        + str(bondParameters[2])
        + ", "
        + str(bondParameters[3])
        + ")"
    )
    print("")
...
if num_a_bonds == 0:
    if num_a_angles == 0:
        if num_a_torsions == 0:
            alchemical_region_A = openmmtools.alchemy.AlchemicalRegion(
                alchemical_atoms=ligand_a_atoms, name="A"
            )
        else:
            alchemical_region_A = openmmtools.alchemy.AlchemicalRegion(
                alchemical_atoms=ligand_a_atoms,
                alchemical_torsions=ligand_a_torsions,
                name="A",
            )
    else:
        if num_a_torsions == 0:
            alchemical_region_A = openmmtools.alchemy.AlchemicalRegion(
                alchemical_atoms=ligand_a_atoms,
                alchemical_angles=ligand_a_angles,
                name="A",
            )
        else:
            alchemical_region_A = openmmtools.alchemy.AlchemicalRegion(
    ...
    if num_a_angles == 0:
        if num_a_torsions == 0:
            alchemical_region_A = openmmtools.alchemy.AlchemicalRegion(
                alchemical_atoms=ligand_a_atoms,
                alchemical_bonds=ligand_a_bonds,
                name="A",
            )
        else:
            alchemical_region_A = openmmtools.alchemy.AlchemicalRegion(
...
alchemical_system_in = factory.create_alchemical_system(
    reference_system, alchemical_regions=[alchemical_region_A]
)

alchemical_state_A = openmmtools.alchemy.AlchemicalState.from_system(
    alchemical_system_in, parameters_name_suffix="A"
)
reload(openmmtools.alchemy)
TS = openmmtools.states.ThermodynamicState(
    alchemical_system_in,
    temperature=TMPRT * unit_definitions.kelvin,
    pressure=1 * unit_definitions.bar,
)
...
integrator = LangevinIntegrator(
    TMPRT * unit_definitions.kelvin,
    GAMMA_LN / unit_definitions.picoseconds,
    TSTP * unit_definitions.femtoseconds,
)
...
for k in range(nstates):
    ...
    sampler_states.append(
        openmmtools.states.SamplerState(positions=inpcrd.positions, box_vectors=box_vec)
    )
...
lsd_move = openmmtools.mcmc.LangevinSplittingDynamicsMove(
    timestep=timeStep,
    collision_rate=GAMMA_LN / unit_definitions.picoseconds,
    n_steps=stepsPerIteration,
)
print("Minimizing......")
for k in range(nstates):
...
if runflag == "run":
    repex_simulation = ReplicaExchangeSampler(
        mcmc_moves=lsd_move, number_of_iterations=productionIterations
    )
...
for i in range(0, blocks):
    ...
    print(
        "Relative free energy change for block {0} = {1} +- {2}".format(
            i + 1,
            Delta_f_ij[0, nstates - 1] * kTtokcal,
            dDelta_f_ij[0, nstates - 1] * kTtokcal,
        )
    )
...
Delta_f_ij, dDelta_f_ij = analyzer.get_free_energy()
print(
    "Relative free energy change for the whole {0} = {1} +- {2}".format(
