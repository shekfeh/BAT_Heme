...
def detect_nan_positions(simulation):
    state = simulation.context.getState(getPositions=True)
    pos = state.getPositions(asNumpy=True)
    for i, xyz in enumerate(pos):
        if any(
            coord != coord or abs(coord.value_in_unit(nanometer)) > 1e6 for coord in xyz
        ):
            print(f"❌ Atom {i} has invalid position: {xyz}")
            raise RuntimeError("NaN or overflow detected in positions!")
...
for i in range(0, len(lines)):
    if (lines[i][0:6].strip() == "ATOM") or (lines[i][0:6].strip() == "HETATM"):
        if lines[i][17:20].strip() == "UNL" or lines[i][17:20].strip() == "UNL":
...
detect_nan_positions(simulation)
...
for step in range(0, n_total_steps, n_check_interval):
    simulation.step(n_check_interval)
    detect_nan_positions(simulation)
