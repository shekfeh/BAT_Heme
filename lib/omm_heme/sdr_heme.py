...
# === Fe-Heme-related restraints ===
print("Fe-heme harmonic restraints:")
print("")

for atoms, eq, k in zip(atoms_fe, eq_fe, fcn_fe):
    bondlength = eq * unit_definitions.angstroms
    bondforce = (
        k * unit_definitions.kilocalorie_per_mole / unit_definitions.angstroms**2
    )
    harmonicforce.addBond(atoms[0], atoms[1], [bondlength, bondforce])
    print(
        f"harmonicforce.addBond({atoms[0]}, {atoms[1]}, "
...
alchemical_region_B = openmmtools.alchemy.AlchemicalRegion(
    alchemical_atoms=ligand_b_atoms, name="B"
)
alchemical_system_in = factory.create_alchemical_system(
    reference_system, alchemical_regions=[alchemical_region_A, alchemical_region_B]
)
...
alchemical_state_B = openmmtools.alchemy.AlchemicalState.from_system(
    alchemical_system_in, parameters_name_suffix="B"
)
reload(openmmtools.alchemy)
