...
def restraints(
        ...
        with open("./vac.pdb") as f_in:
            ...
            for i in range(0, len(lines)):
                if (lines[i][0:6].strip() == "ATOM") or (
                    ...
                    if int(lines[i][22:26].strip()) >= 2 and int(
                        lines[i][22:26].strip()
                    ) < int(lig_res):
                ...
                with open("./vac.pdb") as f_in:
                    ...
                    for i in range(0, len(lines)):
                        if (lines[i][0:6].strip() == "ATOM") or (
                            ...
                            if (
                                int(lines[i][22:26].strip()) >= 3
                                and int(lines[i][22:26].strip()) <= rec_res
                            ):
                                data = lines[i][12:16].strip()
                                if (
                                    data == "CA"
    ...
    for i in range(0, len(mlines)):
        ...
        if int(data[3]) > 0:
            ...
            msk.append(
                "%s %s %s %s"
                % (
                    ligand_atm_num[anum[0]],
                    ligand_atm_num[anum[1]],
                    ligand_atm_num[anum[2]],
                    ligand_atm_num[anum[3]],
                )
            )
        ...
        if len(data) > 7:
            if int(data[8]) > 0:
                ...
                msk.append(
                    "%s %s %s %s"
                    % (
                        ligand_atm_num[anum[5]],
                        ligand_atm_num[anum[6]],
                        ligand_atm_num[anum[7]],
                        ligand_atm_num[anum[8]],
                    )
                )
    ...
    for i in range(0, len(excl)):
        ...
        for j in range(0, len(excl)):
            if j == i:
                break
            data2 = excl[j].split()
            if (data[1] == data2[1] and data[2] == data2[2]) or (
                data[1] == data2[2] and data[2] == data2[1]
            ):
                ind = 0
    ...
    # -------------------------------
    # ✅ DEBUG: Print anchors before writing assign.in
    # -------------------------------
    print("🧷 Parsed anchor atoms:")
    print("  P1 =", P1)
    print("  P2 =", P2)
    print("  P3 =", P3)
    print("  L1 =", L1)
    ...
    # -------------------------------
    # Write assign.in for cpptraj
    # -------------------------------
    assign_file = open("assign.in", "w")
    assign_file.write(
        "%s  %s  %s  %s  %s  %s  %s\n" % ("# Anchor atoms", P1, P2, P3, L1, L2, L3)
    )
    assign_file.write("parm full.prmtop\n")
    ...
    for i in range(0, len(rst)):
        ...
        if len(arr) == 2:
            assign_file.write(
                "%s %s %s\n" % ("distance r" + str(i), rst[i], "noimage out assign.dat")
            )
        elif len(arr) == 3:
            assign_file.write(
                "%s %s %s\n" % ("angle r" + str(i), rst[i], "out assign.dat")
            )
        elif len(arr) == 4:
            assign_file.write(
                "%s %s %s\n" % ("dihedral r" + str(i), rst[i], "out assign.dat")
            )
        else:
            print(
                f"⚠️ Warning: skipping rst[{i}] — unexpected number of atoms: {len(arr)}"
            )
            skipped += 1

    assign_file.close()
    print(f"✅ assign.in written with {len(rst) - skipped} usable restraints.")
    ...
    if len(lines) < 2:
        raise RuntimeError(
            "❌ assign.dat does not contain enough lines — cpptraj likely failed. Check assign.log."
        )
    ...
    if len(vals) < len(rst):
        print(
            f"⚠️ Warning: only {len(vals)} values for {len(rst)} restraints. Some anchors likely invalid."
        )

    # Check restraints in assign.in
    print("\n🔍 Checking restraint masks in assign.in against prmtop...")

    # Read assign.in to isolate each restraint mask
    ...
    if comp != "c" and comp != "r" and comp != "w" and comp != "f":
        ...
        for i in range(0, len(rst)):
            ...
            if i < 3:
                if stage != "equil":
                    if len(data) == 2:
                        nums = (
                            str(atm_num.index(data[0]))
                            + ","
                            + str(atm_num.index(data[1]))
                            + ","
                        )
                ...
                if comp == "e":
                    if i == (3 + nd):
                        nums2 = (
                            str(atm_num.index(data[0]))
                            + ","
                            + str(atm_num.index(data[1]) + vac_atoms)
                            + ","
                        )
                    ...
                    if i == (5 + nd):
                        nums2 = (
                            ...
                            + str(atm_num.index(data[2]))
                            + ","
                            + str(atm_num.index(data[3]) + vac_atoms)
                            + ","
                        )
                        disang_file.write("%s %-23s " % ("&rst iat=", nums2))
                        disang_file.write(
            ...
            # Ligand conformation (non-hydrogen dihedrals)
                ...
                if len(data) == 4:
                    ...
                    if comp == "v" and (
                        ...
                        nums2 = (
                            str(atm_num.index(data[0]) + vac_atoms)
                            + ","
                            + str(atm_num.index(data[1]) + vac_atoms)
                            + ","
                            + str(atm_num.index(data[2]) + vac_atoms)
                    ...
                    if comp == "x" and dec_method == "exchange":
                        nums2 = (
                            str(atm_num.index(data[0]) + vac_atoms + 2 * ref_atoms)
                            + ","
                            + str(atm_num.index(data[1]) + vac_atoms + 2 * ref_atoms)
                            + ","
                            + str(atm_num.index(data[2]) + vac_atoms + 2 * ref_atoms)
                            + ","
                            + str(atm_num.index(data[3]) + vac_atoms + 2 * ref_atoms)
                            + ","
                        )
                        disang_file.write("%s %-23s " % ("&rst iat=", nums2))
                        disang_file.write(
                    ...
                    if comp == "e":
                        ...
                        if dec_method == "sdr" or dec_method == "exchange":
                            nums3 = (
                                str(atm_num.index(data[0]) + 2 * vac_atoms)
                                + ","
                                + str(atm_num.index(data[1]) + 2 * vac_atoms)
                                + ","
                                + str(atm_num.index(data[2]) + 2 * vac_atoms)
                                + ","
                                + str(atm_num.index(data[3]) + 2 * vac_atoms)
                                + ","
                            )
                            ...
                            nums4 = (
                                str(atm_num.index(data[0]) + 3 * vac_atoms)
                                + ","
                                + str(atm_num.index(data[1]) + 3 * vac_atoms)
                                + ","
                                + str(atm_num.index(data[2]) + 3 * vac_atoms)
                                + ","
                                + str(atm_num.index(data[3]) + 3 * vac_atoms)
                                + ","
                            )
                ...
                if len(arr) == 2:
                    restraints_file.write(
                        ...
                        % (
                            "distance d" + str(i),
                            rst[i],
                            "noimage out restraints.dat\n",
                        )
                    )
                if len(arr) == 3:
                    restraints_file.write(
                        "%s %s %s"
                        % ("angle a" + str(i), rst[i], "out restraints.dat\n")
                    )
                if len(arr) == 4:
                    restraints_file.write(
                        "%s %s %s"
                        % ("dihedral a" + str(i), rst[i], "out restraints.dat\n")
                    )
        elif comp == "a":
            restraints_file = open("restraints.in", "w")
            restraints_file.write(
        ...
        for i in range(0, len(rst)):
            ...
            if len(data) == 2:
                nums = (
                    str(ligand_atm_num.index(data[0]))
                    + ","
                    + str(ligand_atm_num.index(data[1]))
                    + ","
                )
                nums2 = (
                    str(ligand_atm_num.index(data[0]) + vac_atoms)
                    + ","
                ...
                nums = (
                    ...
                    + str(ligand_atm_num.index(data[1]))
                    + ","
                    + str(ligand_atm_num.index(data[2]))
                    + ","
                    + str(ligand_atm_num.index(data[3]))
                    + ","
                )
                nums2 = (
                    str(ligand_atm_num.index(data[0]) + vac_atoms)
                    ...
                    + str(ligand_atm_num.index(data[1]) + vac_atoms)
                    + ","
                    + str(ligand_atm_num.index(data[2]) + vac_atoms)
                    + ","
                    + str(ligand_atm_num.index(data[3]) + vac_atoms)
                    + ","
                )
                disang_file.write("%s %-23s " % ("&rst iat=", nums))
                disang_file.write(
        ...
        for i in range(0, len(rst)):
            ...
            if len(arr) == 2:
                restraints_file.write(
                    "%s %s %s"
                    % ("distance d" + str(i), rst[i], "noimage out restraints.dat\n")
                )
            if len(arr) == 3:
                restraints_file.write(
                    "%s %s %s" % ("angle a" + str(i), rst[i], "out restraints.dat\n")
                )
            if len(arr) == 4:
                restraints_file.write(
                    "%s %s %s" % ("dihedral a" + str(i), rst[i], "out restraints.dat\n")
                )
    ...
    if comp == "x":
        ...
        for i in range(0, len(rst)):
            ...
            if len(arr) == 2:
                assign_file.write(
                    "%s %s %s"
                    % ("distance r" + str(i), rst[i], "noimage out assign2.dat\n")
                )
            if len(arr) == 3:
                assign_file.write(
                    "%s %s %s" % ("angle r" + str(i), rst[i], "out assign2.dat\n")
                )
            if len(arr) == 4:
                assign_file.write(
                    "%s %s %s" % ("dihedral r" + str(i), rst[i], "out assign2.dat\n")
                )
        ...
        for i in range(0, len(rst)):
            ...
            # Ligand conformational restraints
                ...
                if len(data) == 4:
                    ...
                    nums2 = (
                        str(atm_num.index(data[0]) + ref_atoms)
                        + ","
                        + str(atm_num.index(data[1]) + ref_atoms)
                        + ","
                        + str(atm_num.index(data[2]) + ref_atoms)
                        + ","
                        + str(atm_num.index(data[3]) + ref_atoms)
                        + ","
                    )
                    disang_file.write("%s %-23s " % ("&rst iat=", nums))
                    disang_file.write(
...
def sim_files(
    hmr, temperature, mol, num_sim, pose, comp, win, stage, steps1, steps2, rng
):
    ...
    if comp != "c" and comp != "r" and comp != "n":
        with open("../amber_files/mini.in", "rt") as fin:
            with open("./mini.in", "wt") as fout:
                for line in fin:
                    fout.write(
                        line.replace("_L1_", L1).replace("_L2_", L2).replace("_L3_", L3)
                    )
        ...
        with open("../amber_files/therm2.in", "rt") as fin:
            with open("./therm2.in", "wt") as fout:
                for line in fin:
                    fout.write(
                        line.replace("_L1_", L1)
                        .replace("_L2_", L2)
                        .replace("_L3_", L3)
                        .replace("_temperature_", str(temperature))
                    )
                    ...
                    if i == (num_sim - 1):
                        for line in fin:
                            fout.write(
                                ...
                                .replace("disang_file", "disang%02d" % int(i))
                            )
                    else:
                        for line in fin:
                            fout.write(
                                line.replace("_temperature_", str(temperature))
                                .replace("_num-atoms_", str(vac_atoms))
                                .replace("_num-steps_", str(steps1))
                                .replace("disang_file", "disang%02d" % int(i))
                            )
                    ...
                    if i == 0:
                        for line in fin:
                            fout.write(
                                ...
                                .replace("_num-atoms_", str(vac_atoms))
                                .replace("_num-steps_", str(steps1))
                                .replace("disang_file", "disang%03d" % int(i))
                            )
                    else:
                        for line in fin:
                            fout.write(
                        ...
                        if i == 1 or i == 0:
                            for line in fin:
                                fout.write(
                                    line.replace("_temperature_", str(temperature))
                                    .replace("_num-atoms_", str(vac_atoms))
                                    .replace("_num-steps_", str(round(steps1 / 2)))
    ...
    if stage == "fe":
        ...
        with open("../run_files/PBS-Am", "rt") as fin:
            with open("./PBS-run", "wt") as fout:
                for line in fin:
                    fout.write(
                        line.replace("STAGE", pose).replace(
                            "POSE", "%s%02d" % (comp, int(win))
                        )
                    )
                        ...
                        if i == 1 or i == 0:
                            for line in fin:
                                fout.write(
                                    ...
                                    .replace("_num-atoms_", str(vac_atoms))
                                    .replace("_num-steps_", str(round(steps1 / 2)))
                                    .replace("lbd_val", "%6.5f" % float(weight))
                                    .replace("mk1", str(mk1))
                                    .replace("mk2", str(mk2))
                                )
                        else:
            ...
            with open("../amber_files/mdin-ch-dd", "rt") as fin:
                with open("./mdin-%02d" % int(i), "wt") as fout:
                    if i == 1 or i == 0:
                        for line in fin:
                            if (
                                not "restraint" in line
                                and not "ntr = 1" in line
                                and not "ntwprt" in line
                                and not "infe" in line
                            ):
                                fout.write(
                                    line.replace("_temperature_", str(temperature))
                                    .replace("_num-atoms_", str(vac_atoms))
