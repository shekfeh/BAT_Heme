...
def restraints(pose, rest, bb_start, bb_end, weight, stage, mol, molr, comp, bb_equil, sdr_dist, dec_method, other_mol):
    ...
    if comp not in ['c', 'r', 'w', 'f']:
      with open('disang.rest', 'w') as disang_file:
        disang_file.write(f"# Anchor atoms {P1} {P2} {P3} {L1} {L2} {L3} stage = {stage} weight = {weight}\n")

        for i, restraint in enumerate(rst):
          data = restraint.split()

          try:
            # Protein conformation (P1-P3 distance restraints)
            if i < 3:
              if stage != 'equil' and len(data) == 2:
                nums = f"{atm_num.index(data[0])},{atm_num.index(data[1])},"
                disang_file.write(f"&rst iat= {nums:<23} ")
                disang_file.write(
                  f"r1= 0.0000, r2= {valse[i]:10.4f}, r3= {valse[i]:10.4f}, r4= 999.0000, "
                  f"rk2= {rdsf:11.7f}, rk3= {rdsf:11.7f}, &end {recep_c}\n"
                )
              elif len(data) == 2:
                nums = f"{atm_num.index(data[0])},{atm_num.index(data[1])},"
                disang_file.write(f"&rst iat= {nums:<23} ")
                disang_file.write(
                  f"r1= 0.0000, r2= {vals[i]:10.4f}, r3= {vals[i]:10.4f}, r4= 999.0000, "
                  f"rk2= {rdsf:11.7f}, rk3= {rdsf:11.7f}, &end {recep_c}\n"
                )

            # Protein conformation (backbone restraints)
            elif 3 <= i < 3 + nd:
              if stage != 'equil' and len(data) == 4:
                nums = ",".join(str(atm_num.index(atom)) for atom in data)
                disang_file.write(f"&rst iat= {nums:<23} ")
                disang_file.write(
                  f"r1= {valse[i] - 180:10.4f}, r2= {valse[i]:10.4f}, r3= {valse[i]:10.4f}, "
                  f"r4= {valse[i] + 180:10.4f}, rk2= {rdhf:11.7f}, rk3= {rdhf:11.7f}, &end {recep_d}\n"
                )
              elif len(data) == 4:
                nums = ",".join(str(atm_num.index(atom)) for atom in data)
                disang_file.write(f"&rst iat= {nums:<23} ")
                disang_file.write(
                  f"r1= {vals[i] - 180:10.4f}, r2= {vals[i]:10.4f}, r3= {vals[i]:10.4f}, "
                  f"r4= {vals[i] + 180:10.4f}, rk2= {rdhf:11.7f}, rk3= {rdhf:11.7f}, &end {recep_d}\n"
                )

            # Ligand translational/rotational restraints
            elif 3 + nd <= i < 9 + nd and comp != 'a':
              if len(data) == 2:
                nums = f"{atm_num.index(data[0])},{atm_num.index(data[1])},"
                disang_file.write(f"&rst iat= {nums:<23} ")
                disang_file.write(
                  f"r1= 0.0000, r2= {vals[i]:10.4f}, r3= {vals[i]:10.4f}, r4= 999.0000, "
                  f"rk2= {ldf:11.7f}, rk3= {ldf:11.7f}, &end {lign_tr}\n"
                )
              elif len(data) == 3:
                nums = ",".join(str(atm_num.index(atom)) for atom in data[:3]) + ","
                disang_file.write(f"&rst iat= {nums:<23} ")
                disang_file.write(
                  f"r1= 0.0000, r2= {vals[i]:10.4f}, r3= {vals[i]:10.4f}, r4= 180.0000, "
                  f"rk2= {laf:11.7f}, rk3= {laf:11.7f}, &end {lign_tr}\n"
                )
              elif len(data) == 4:
                nums = ",".join(str(atm_num.index(atom)) for atom in data)
                disang_file.write(f"&rst iat= {nums:<23} ")
                disang_file.write(
                  f"r1= {vals[i] - 180:10.4f}, r2= {vals[i]:10.4f}, r3= {vals[i]:10.4f}, "
                  f"r4= {vals[i] + 180:10.4f}, rk2= {laf:11.7f}, rk3= {laf:11.7f}, &end {lign_tr}\n"
                )

          except ValueError as ve:
            print(f"Error: Atom not found for restraint '{restraint}' at index {i}. Details: {ve}")
            print(f"Missing atoms in {data} could not be found in atm_num.")
            continue  # Skip this restraint and proceed to the next
