...
def help_message():
    print(
        "Use the flags -i and -s for the input file and current stage of the calculations"
    )
...
def check_tleap():
    ...
    if num_added is None:
        print(
            "Problem solvating system, check your input file and the tleap log file\n"
        )
        sys.exit(1)
    return num_added
...
def box_volume():
    ...
    sp.call("tleap -s -f tmp_tleap.in > tmp.log", shell=True)

    x_axis = y_axis = z_axis = None
    ratio = 1.0  # default scaling ratio
    dens = 1.00  # target density for scaling
    ...
    with open("tmp.log", "r") as f:
        for line in f:
            ...
            if "Density" in line:
                splitdata = line.split()
                try:
                    ratio = float(splitdata[5]) / dens
                except (IndexError, ValueError):
                    print("⚠️ Warning: Failed to extract density — using ratio = 1.0")

    if x_axis is None or y_axis is None or z_axis is None:
        raise ValueError("❌ Could not extract box dimensions from tmp.log")
